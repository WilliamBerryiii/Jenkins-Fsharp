...
module Jenkins =
    ...
    let GetJenkinsRequest resource (configuration:JenkinsConfiguration) = 
        let request = Uri(String.Format("{0}/{1}", configuration.BaseUri, resource))
        printfn "%A" request
        let addAuthorization (configuration:JenkinsConfiguration) request = 
            match (configuration.UserName, configuration.Password) with 
            | ("","") -> request 
            | (_,_) -> withNTLMAuthentication configuration.UserName configuration.Password request
    ...
    let GetResponse (request:Request) =
        let response = async {
            ...
            let body = Async.RunSynchronously (Response.readBodyAsString response)
            
            return response.StatusCode, body
        }

        let result = match Async.RunSynchronously response with 
                        | (200, body) -> Success body
                        | (code, body) -> Failure (String.Format("Status Code:{0}; Body: {1} ", code, body))
        result
    ...
    let GetJobConfig (configuration:JenkinsConfiguration) (name:string) = 
    ...
    let GetVersion (configuration:JenkinsConfiguration) = 
        let response = Async.RunSynchronously (GetJenkinsRequest JenkinsRoutes.Info configuration
                        |> withHeader (Custom ("X-Jenkins", "0.0" ))
    ...
    let CreateJob (configuration:JenkinsConfiguration) (name:string) (config:string) = 
        ...
        let response = PostJenkinsRequest resource configuration 
                        |> withBodyStringEncoded config ASCIIEncoding.UTF8
                        |> GetResponse
    ...
    let ReconfigJob (configuration:JenkinsConfiguration) (name:string) (config:string) =
        ...
        let buildResponse = match response with 
                            | Success s -> Success name
                            | Failure f -> Failure f 
        buildResponse

    let BuildJob (configuration:JenkinsConfiguration) parameters (token : string option) (name:string)  =
        let resource = String.Format(JenkinsRoutes.BuildJob, name)
        let request = PostJenkinsRequest resource configuration 

        parameters |> Seq.iter (fun (x,y) -> request 
                                          |> withQueryStringItem x y
                                          |> ignore)

        match token with
        | Some token -> request |> withQueryStringItem "token" token |> ignore
        | None -> ()
